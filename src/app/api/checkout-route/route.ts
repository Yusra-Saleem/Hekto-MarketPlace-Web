...
export async function POST(request: Request) {
  try {
    const { cartItems, labelPrice, shippingRate, tax } = await request.json();
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `http://localhost:3000/order-completed?session_id={CHECKOUT_SESSION_ID}`, // Redirect URL if payment is successful
